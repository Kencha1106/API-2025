...
1.  **背景・業務領域の具体化**
   ...
   　要件定義との変更点は今回の実測値（性能測定）の追加を行った
  
2.  **課題・目的・APIが必要な理由**
 課題について：
- 学習ログが属人化し、データが分散しているため
- 計画と実績の差分が見えないため
- 過去データの検索・分析が困難なため
APIが必要な理由
- データを統一フォーマットで保存できる
...
4.  **業務フロー・ユースケース**
5.  *   Mermaid図で**2件以上の主要ユースケース**を実装に対応づけ（ID付与）
    sequenceDiagram
    actor User
...
9.  **まとめ**
今回私が作成したAPIは、レポート04で定義した「学習進捗管理の課題」を、
FastAPI + Streamlit + OpenAPI の使用で解決に向けた。
- 要件 → 設計 → 実装 → 検証 の一貫性を確保した
- CRUD + 検索 + 認証 + レート制限を実装した
製作での困難な点
- 要件定義と OpenAPI の整合性維持
- 認証（JWT）と認可（権限モデル）の実装が複雑だった
- 学生・管理者・外部アプリで権限が異なるため、
403 Forbidden の条件分岐 が増えた。
以降のサービスの改善点
リマインダー / 通知機能（外部サービス連携）
- 計画した時間に通知をする
- 進捗が遅れている場合にも通知を行う
- Slack / LINE Notify / Discord Webhook などと連携を予定
