...
---一つは200、リクエストが成功した時に帰ってくる通常のレスポンス。設計での利用例はGET/usersでユーザー一覧を返すときに利用される。
一つは301、URLを永続的に新しいものへ移動させるときに帰ってくる。設計での利用例はAPIのバージョンを古いエンドポイントから新しいものに変え誘導するときに利用される。
一つは400、指定したリソースが見つからない場合に帰ってくる。設計での利用例は間違ったJSON構造をしているときに帰ってくる。
