...
---
一つは200、リクエストが成功した時に帰ってくる通常のレスポンスを表している。設計での利用例はGET/usersでユーザー一覧を返すときに利用される。
具体的な例は、
一つは301、URLを永続的に新しいものへ移動させるときに帰ってくるもの。設計での利用例はAPIのバージョンを古いエンドポイントから新しいものに変え誘導するときに利用される。
一つは400、指定したリソースが見つからない場合に帰ってくるもの。設計での利用例は間違ったJSON構造をしているときに帰ってくる。
...
---・軽量性・シンプルさ
   ・JavaScriptとの親和性
    ・可読性・扱いやすさ
- JavaScriptの script タグで外部ドメインからデータを取得する仕組み。クロスドメイン通信を可能にするため。任意のJSのコードが実行されるためXSS（クロスサイトスクリプティング）攻撃のリスクが常にある
...
エラー原因の分類 → 認証、権限、リソース不在、入力エラー、内部エラー、レート制限など
