...
---
WebAPIのレスポンス形式としてJSONが主流になった理由は、XMLと比べて以下の点が優れているためである。
・記述が簡潔：JSONはタグを持たず、XMLよりもデータ量が少なく可読性が高い。
・JavaScriptとの親和性：JSONはJavaScriptのオブジェクト記法そのものであり、パースや扱いが容易で、フロントエンド実装に向いている。
・処理速度が速い：構造が単純なため、XMLに比べてパース処理が軽量で高速である。
・学習コストが低い：構造が直感的で、初心者にも理解しやすい。

JSONP（JSON with Padding）は、<script>タグで外部ドメインのAPIを読み込み、コールバック関数としてJSONを実行させる仕組みである。主に同一オリジンポリシー回避のために利用された。
しかし、任意のJavaScriptが実行されるため、XSSの危険性があり、GET通信のみ対応など制約も多い。そのため現在はCORSが主流である。
...
---
APIのエラー情報設計では、HTTPステータスコードに加えて内容を明確に伝えることが重要です。
・開発者向けメッセージと利用者向けメッセージは分離すべきである。開発者向けには原因特定に役立つ技術的な情報（エラーコード、内部的な失敗理由）を返し、利用者向けには「再ログインしてください」「入力内容を確認してください」など、行動につながる簡潔で安全な表現を用いる。内部構造や脆弱性につながる情報は利用者向けに含めない。

・エラー原因の分類が重要である。認証エラー（401）、権限不足（403）、リソース不在（404）、入力不正（400）、サーバエラー（500）などを明確に区別することで、クライアント側が適切なリトライや画面制御を行える。

・ドキュメントURLの付与は、エラーの意味や対処方法を即座に参照でき、実装者の調査コストを下げる。結果として保守性と開発効率が向上する。
